...
!--- compute air density at full and half levels
  ...
  rho_half(:,:,kx+1) = p_half(:,:,kx+1)/(rdgas*tv(:,:,kx))  ! though rho_half(kx+1) should not be used,
...
!------------------------------
      do k=1,kxp
        ...
        do n=1,nx
          ...
          each_up_massflux(n,i,j,kk) = rho_half(i,j,kk) * up_a(n,k)*up_w(n,k)
      ...
      do k=1,kxp
        sum_up_massflux(i,j,k) = rho_half(i,j,k) * sum_up_aw(i,j,k)
      enddo
...
!------------------------------
      ...
      if (is_mass_flux(i,j).eq.1.) then  ! there is mass flux
        ...
        call mass_flux_tendencies (option_solver, dt, &
                                   z_half, rho_full, rho_half, sum_up_aw, &
...
if (do_printout_ncl) then
  ...
  print*,';   dt=',dt
  print*,''
  print*,';    remove the space at the beginning	: [s/^ */  /g]'
  print*,';    add "/)" at the end              	: [ s/$/\/)/g]'
  print*,';    add comma            		: [s/   /,  /g]'
  print*,';    replace (/, with (/  		: [s/\/,/\//g]'
  print*,';    replace ", ," to "," 		: [s/, *,/,/g]'
  print*,';    replan then/) to then		: [s/then\/)/then/g]'
  print*,';    replan end if/) to end if		: [s/end if\/)/end if/g]'
  print*,';---------------------------'
  print*,''
  print*,';  wks_name = ""'
  print*,';  data_source = ""'
  ...
  i=1
  j=1
  phi_half(1) = thv(i,j,1)
  do k=2,kx
    phi_half(k) = 0.5* (thv(i,j,k) + thv(i,j,k-1))
  enddo
  write(*,1001) 'thv_env_half = (/',phi_half
  ...
  write(*,1001) 'sum_up_aw = (/',sum_up_aw
  write(*,1001) 'sum_up_awthv = (/',sum_up_awthv
...
!---------------------------------------------------------------------
  ...
  real,  intent(in), dimension(:,:,:) :: &  ! dimension (nlon, nlat, nlay+1)
    z_half, rho_half, sum_up_aw, sum_up_awphi
...
!---------------------------------------------------------------------
  ...
  !--- working variables
  real, dimension(size(phi,1),size(phi,2),size(phi,3)+1) :: &  ! dimension (nlon, nlat, nlay+1) 
...
!--- initialize work variables 
  sum_up_raw   (:,:,:) = rho_half(:,:,:) * sum_up_aw   (:,:,:)
  sum_up_rawphi(:,:,:) = rho_half(:,:,:) * sum_up_awphi(:,:,:)
...
!-------------------
  if (option_solver .eq. "explicit") then
    ...
    do j=1,jx
    do k=2,kx
      sum_env_rawphi(i,j,k) = rho_half(i,j,k) * sum_up_aw(i,j,k) * 0.5*(phi(i,j,k-1)+phi(i,j,k))
    enddo
    enddo
    enddo
...
!
    ...
    do j=1,jx
    do k=kx,1,-1
      phidt_mf(i,j,k) = -1./rho_full(i,j,k) * (rho_w1_phi1(i,j,k)-rho_w1_phi1(i,j,k+1))  &
...
!          rho_w1_phi1(i,j,k) = -phidt_mf(i,j,k)*rho_full(i,j,k)*(z_half(i,j,k) - z_half(i,j,k+1)) & 
!                              + rho_w1_phi1(i,j,k+1)
...
!-----------------------
  ...
  else if (option_solver .eq. "explicit") then
    phi_new    (:,:,:) = phi   (:,:,:)
  end if
  ...
  do j=1,jx
  do k=2,kx
    sum_env_rawphi(i,j,k) = rho_half(i,j,k) * sum_up_aw(i,j,k) * 0.5*(phi_new(i,j,k)+phi_new(i,j,k-1))
  enddo
  enddo
  enddo
  ...
  !--- compute w'phi'
  w1_phi1(:,:,1:kx) = rho_w1_phi1(:,:,1:kx) / rho_half(:,:,1:kx) 
...
if (do_check_lhs_rhs) then
  ...
  do k=1,kx
    lhs = phidt_mf(i,j,k)
    rhs = -1./rho_full(i,j,k) * (rho_w1_phi1(i,j,k)-rho_w1_phi1(i,j,k+1))  &
                              / (z_half(i,j,k) - z_half(i,j,k+1))
