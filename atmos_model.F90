...
public yhc_get_atmos_model_fields ! yhc add
!-----------------------------------------------------------------------
...
!--- indices for one-based arrays
       ...
       call physics_driver_down ( is, ie, js, je, npz, Time_prev, Atmos%Time, Time_next, &
                                  ...
                                  Moist_clouds_block =  Moist_clouds(1)%block(blk), &
                                  lprec = Atmos%lprec(is:ie,js:je), &
                                  fprec = Atmos%fprec(is:ie,js:je) )
                                  !Rad_flux(1)%block(blk) )

                                !Rad_flux(1)%block(blk), &
                                !shflx = Surface_boundary%shflx    (isw:iew,jsw:jew), &  !yhc
                                !lhflx = Surface_boundary%lhflx    (isw:iew,jsw:jew) )  !yhc
    enddo
...
!<--- yhc
! At least one of these went wrong...  09-14-2020
...
!<--- yhc add, to get atmospheric variables 
!subroutine update_atmos_model_down( Surface_boundary, Atmos )
subroutine yhc_get_atmos_model_fields( Surface_boundary, Atmos, string00, do_check_column )
!
!-----------------------------------------------------------------------
  type(land_ice_atmos_boundary_type), intent(inout) :: Surface_boundary
  type (atmos_data_type), intent(inout) :: Atmos
  logical :: do_check_column
