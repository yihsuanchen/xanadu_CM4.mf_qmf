...
! yihsuan add
       id_k_wentr_pbl = register_diag_field (mod_name,'k_wentr_pbl',       &
            axes(1:2), time,                                           &
            'Entrainment heat diffusivity coefficient from surface buoyancy flux',         &
            'meters squared per second', missing_value=missing_value )
...
real, dimension(size(t,1),size(t,2))            :: wentr_rad,wentr_pbl,k_wentr_pbl ! yihsuan
...
!      initialize variables
       ...
       convpbl    = 0.0
       k_wentr_pbl  = missing_value  ! yihsuan
...
! yihsuan
  ! write(6,*) '-----------------'
  ! write(6,*) 'ggg0'
  ! write(6,*) 'ipbl,z(ipbl-1),z(ipbl)',ipbl,zfull_ag(i,j,ipbl-1),zfull_ag(i,j,ipbl)
  ! write(6,*) 'wentr_tmp,k_entr_tmp',wentr_tmp,k_entr_tmp
  ! write(6,*) '-----------------'
         ...
         k_wentr_pbl(i,j)     = k_entr_tmp  ! yihsuan
...
!
       ...
       if ( id_convpbl    > 0 .or. id_wentr_pbl > 0 .or.               &
          ...
          if ( id_k_wentr_pbl > 0 ) then
               used = send_data ( id_k_wentr_pbl, k_wentr_pbl, time,     &
                          is, js)
          end if
