...
!--> yi-hsuan add mass_flux module
use mass_flux_mod,           only: mass_flux_driver, mass_flux_init, mass_flux, &
                                   mass_flux_end, mf_tendency_type
!<-- yi-hsuan
...
logical :: do_moist_processes_up     = .true.
logical :: do_moist_processes_down   = .false.

logical :: do_EDMF_in_mass_flux      = .false.  ! this is a global variable
!--> yi-hsuan 
...
namelist / physics_driver_nml / do_radiation, do_clubb,  do_cosp, &
                                ...
                                do_moist_processes_up, do_moist_processes_down, &
...
integer :: damping_clock, turb_clock,   &
           ...
           moist_processes_clock, cosp_clock, mass_flux_clock  ! yi-hsuan add mass_flux_clock
...
!  local variables:
      ...
      integer          ::  moist_processes_init_clock, damping_init_clock,&
                           ...
                           mass_flux_init_clock,  & ! yi-hsuan add
                           tracer_init_clock
...
!<--- yhc
      if (do_mass_flux_before_vdiff .and. do_mass_flux_after_vdiff) then
         call error_mesg('physics_driver_init',  &
         'do_mass_flux_before_vdiff and do_mass_flux_after_vdiff cannot be true at the same time', FATAL)
      endif

      if (do_mass_flux_diagnostic) then
        if (.not.do_mass_flux_before_vdiff .and. .not.do_mass_flux_after_vdiff) then
           call error_mesg('physics_driver_init',  &
           'When do_mass_flux_diagnostic is .true., either do_mass_flux_before_vdiff or do_mass_flux_after_vdiff must be true', FATAL)
        endif
      endif

      if (do_moist_processes_down .and. do_moist_processes_up) then
         call error_mesg('physics_driver_init',  &
         'do_moist_processes_down and do_moist_processes_up cannot be true at the same time', FATAL)
      endif
!---> yhc
...
!<-- yi-hsuan add mass_flux_clock, and mass_flux_init_clock
      mass_flux_clock            =      &
                mpp_clock_id( '   Physics_down/up: Mass Flux', &
                  grain=CLOCK_MODULE_DRIVER )
      mass_flux_init_clock            =      &
        mpp_clock_id( '   Physics_driver_init: Mass Flux: Initialization', &
                  grain=CLOCK_MODULE_DRIVER )
!--> yi-hsuan add mass_flux_clock
...
!--> yi-hsuan debug, 2020-07-02
  id_tdt_before_mf = register_diag_field (mod_name, 'tdt_before_mf', axes(1:3), Time, &
                 't tendency before mass flux', 'K/s' , &
                 missing_value=missing_value )

  id_tdt_after_mf = register_diag_field (mod_name, 'tdt_after_mf', axes(1:3), Time, &
                 't tendency after mass flux', 'K/s' , &
                 missing_value=missing_value )

  id_tdt_after_vdiff_down = register_diag_field (mod_name, 'tdt_after_vdiff_down', axes(1:3), Time, &
                 't tendency after vert diff down', 'K/s' , &
                 missing_value=missing_value )

  id_tdt_after_vdiff_up = register_diag_field (mod_name, 'tdt_after_vdiff_up', axes(1:3), Time, &
                 't tendency after vert diff up', 'K/s' , &
                 missing_value=missing_value )

  ! diff_t in physics_driver is at full levels , but diff_t in vert_turb is at half levels!?
  !! write(6,*) 'gggX axes',axes
  id_diff_t_smooth = register_diag_field (mod_name, 'diff_t_smooth', axes(1:3), Time, &
                 'Heat diffusion coeffs after smoothing', 'm2/s' , &
                 missing_value=missing_value )

  id_diff_m_smooth = register_diag_field (mod_name, 'diff_m_smooth', axes(1:3), Time, &
                 'Momentum diffusion coeffs after smoothing', 'm2/s' , &
                 missing_value=missing_value )
  id_diff_t_vert = register_diag_field (mod_name, 'diff_t_vert', axes(1:3), Time, &
                 'Heat diffusion coeffs in physics_down', 'm2/s' , &
                 missing_value=missing_value )

  id_diff_m_vert = register_diag_field (mod_name, 'diff_m_vert', axes(1:3), Time, &
                 'Momentum diffusion coeffs in physics_down', 'm2/s' , &
                 missing_value=missing_value )
...
subroutine physics_driver_down (is, ie, js, je, npz,              &
                                ...
                                Rad_flux_control,                 &
                                Rad_flux_block,                   &
                                diffm, difft, Moist_clouds_block, lprec, fprec)
                                !diffm, difft  )
                                !shflx, lhflx,   & ! yihsuan add
...
!real,dimension(:,:),    intent(in), optional    :: shflx, lhflx  ! yihsuan add
...
!---> yi-hsuan, 2020-05-23
!      type(mf_tendency_type)                 :: Tend_mf    
      integer                                :: id, jd, kd 
      integer                                :: i,j,k      
      !logical :: do_EDMF_in_mass_flux
      real, dimension(ie-is+1,je-js+1,npz)   :: diff_t_to_vdiff, diff_m_to_vdiff
      ...
      !--- output argument ---
      type(clouds_from_moist_block_type), intent(inout), optional :: Moist_clouds_block
      real, dimension(ie-is+1, je-js+1), intent(inout), optional      :: &
        lprec, fprec
      ...
      real, dimension(ie-is+1, je-js+1, npz+1)   :: &
        pflux
      real, dimension(ie-is+1, je-js+1)      :: &
        gust_cv
      real, dimension(ie-is+1, je-js+1), target  :: &
        tdt_shf,  qdt_lhf
...
!      Tend_mf         derived type variable for mass_flux tendencies 
...
!  endif
  ...
  ! write(6,*) 'physics_down, beginning, v,',v
  ! write(6,*) 'physics_down, beginning, t,',t
  ! write(6,*) 'physics_down, beginning, q,',r(:,:,:,1)
  ! write(6,*) 'physics_down, beginning, udt,',udt
  ! write(6,*) 'physics_down, beginning, vdt,',vdt
  ! write(6,*) 'physics_down, beginning, tdt,',tdt
  ! write(6,*) 'physics_down, beginning, qdt,',rdt(:,:,:,1)
...
!-----------------------------------------------------------------------
!    allocate and initialize Tend_mf variables
!-----------------------------------------------------------------------
...
!<-- flag111
  ! write(6,*) 'physics_down, bc   damp, u,',u
  ! write(6,*) 'physics_down, bc   damp, v,',v
  ! write(6,*) 'physics_down, bc   damp, t,',t
  ! write(6,*) 'physics_down, bc   damp, q,',r(:,:,:,1)
  ! write(6,*) 'physics_down, bc   damp, udt,',udt
  ! write(6,*) 'physics_down, bc   damp, vdt,',vdt
  ! write(6,*) 'physics_down, bc   damp, tdt,',tdt
  ! write(6,*) 'physics_down, bc   damp, qdt,',rdt(:,:,:,1)
!<-- flag111
...
!<--flag11
  ! write(6,*) 'physics_down, vert_turb, u,',u
  ! write(6,*) 'physics_down, vert_turb, v,',v
  ! write(6,*) 'physics_down, vert_turb, t,',t
  ! write(6,*) 'physics_down, vert_turb, q,',r(:,:,:,1)
  ! write(6,*) 'physics_down, vert_turb, udt,',udt
  ! write(6,*) 'physics_down, vert_turb, vdt,',vdt
  ! write(6,*) 'physics_down, vert_turb, tdt,',tdt
  ! write(6,*) 'physics_down, vert_turb, qdt,',rdt(:,:,:,1)
!<-- flag11
...
!<--- flag11
  ! write(6,*) 'physics_down, bc tracer, u,',u
  ! write(6,*) 'physics_down, bc tracer, v,',v
  ! write(6,*) 'physics_down, bc tracer, t,',t
  ! write(6,*) 'physics_down, bc tracer, q,',r(:,:,:,1)
  ! write(6,*) 'physics_down, bc tracer, udt,',udt
  ! write(6,*) 'physics_down, bc tracer, vdt,',vdt
  ! write(6,*) 'physics_down, bc tracer, tdt,',tdt
  ! write(6,*) 'physics_down, bc tracer, qdt,',rdt(:,:,:,1)
!<--- flag11
...
!<--- flag11
! write out diff
! write(6,*) 'ggg1 original diff_t',diff_t_vert
! write(6,*) 'ggg2 smooth   diff_t',diff_t
!! write(6,*) 'ggg2 original diff_m',diff_m_vert
!! write(6,*) 'ggg2 smooth   diff_m',diff_m
!<--- flag11
...
!<-- yihsuan 
       ...
       !--- output smoothed diffusion coefficients, which are actually used in vert_diff 
       !      Note that the dimension of diff_t and diff_m is (nlon, nlat, nlay), 
       !      though in vert_turb_driver, the dimension of diff_t and diff_m is (nlon, nlat, nlay+1).
       if ( id_diff_t_smooth > 0) then
         used = send_data (id_diff_t_smooth, diff_t, Time_next, is, js, 1 )
       endif

       if ( id_diff_m_smooth > 0) then
         used = send_data (id_diff_m_smooth, diff_m, Time_next, is, js, 1 )
       endif

       if ( id_diff_t_vert > 0) then
         used = send_data (id_diff_t_vert, diff_t_vert, Time_next, is, js, 1 )
       endif

       if ( id_diff_m_vert > 0) then
         used = send_data (id_diff_m_vert, diff_m_vert, Time_next, is, js, 1 )
       endif

!---------------------------------------------------------------------
!    call mass_flux to to calculate tendencies from convective mass flux
!---------------------------------------------------------------------

     if (do_mass_flux_before_vdiff) then

       !------- output t tendency before mass flux (units: K/s) at full level -------
       if ( id_tdt_before_mf > 0) then
         used = send_data (id_tdt_before_mf, tdt, Time_next, is, js, 1 )
       endif

       call mpp_clock_begin ( mass_flux_clock )

       call mass_flux_driver ( is, ie, js, je, dt, Time_next,           &
 ...
 if (do_writeout_column) then
    ...
    write(6,*) 'u_star',u_star(ii_write,jj_write)
    write(6,*) 'q_star',q_star(ii_write,jj_write)
        write(6,3001) 'data t_massflux/'    ,t(ii_write,jj_write,:)
        write(6,3002) 'data q_massflux/'    ,r(ii_write,jj_write,:,1)
        !write(6,3002) 'data tdt_tracer/'    ,tdt(ii_write,jj_write,:)
        !write(6,3002) 'data qdt_tracer/'    ,rdt(ii_write,jj_write,:,1)
        write(6,*) 'data tdt_massflux/'    ,tdt(ii_write,jj_write,:)
        write(6,*) 'data qdt_massflux/'    ,rdt(ii_write,jj_write,:,1)
       write(6,*) '-------------- i,j,',ii_write,jj_write
  endif

      !------- output t tendency after mass flux (units: K/s) at full level -------
      if ( id_tdt_after_mf > 0) then
        used = send_data (id_tdt_after_mf, tdt, Time_next, is, js, 1 )
      endif

       call mpp_clock_end   ( mass_flux_clock )
     end if ! end if of do_mass_flux_before_vdiff

!--> yi-hsuan


!*** put moist_processes here ***
if (do_moist_processes_down) then

     !---
     ! tdt_shf          temperature tendency from sensible heat flux
     ! qdt_lhf          moisture tendency from latent heat flux
     ! These two are zero before vert_diff_driver_up
     tdt_shf(:,:) = 0. 
     qdt_lhf(:,:) = 0.
...
!-----------------------------------------------------------------------
  ...
  if (do_writeout_column) then
    ...
    write(6,*) 'u_star',u_star(ii_write,jj_write)
    write(6,*) 'q_star',q_star(ii_write,jj_write)
        write(6,3001) 'data t_moist_down/'    ,t(ii_write,jj_write,:)
        write(6,3002) 'data q_moist_down/'    ,r(ii_write,jj_write,:,1)
        write(6,3002) 'data tdt_moist_down/'    ,tdt(ii_write,jj_write,:)
        write(6,3002) 'data qdt_moist_down/'    ,rdt(ii_write,jj_write,:,1)
       write(6,*) '-------------- i,j,',ii_write,jj_write
  endif
...
!    write(6,*) 'evap     ',lhflx(ii_write,jj_write)
!    write(6,*) 'evap_star',evap_star(ii_write,jj_write)
...
!<--- flag11
  ...
  ! write(6,*) 'physics_up  , mosit    , q,',r(:,:,:,1)
  ! write(6,*) 'physics_up  , moist    , tdt,',tdt
  ! write(6,*) 'physics_up  , moist    , qdt,',rdt(:,:,:,1)
!<--- flag11
...
!  write(6,*) '-------------------------'
!  write(6,*) 'gg1, moist_processes, physics_down'
!  !write(6,*) 'Moist_clouds_block',Moist_clouds_block
!  !write(6,*) 'Physics_tendency_block',Physics_tendency_block
!  !write(6,*) 'Phys_mp_exch',Phys_mp_exch
!  write(6,*) 'lprec',lprec
!  write(6,*) 'fprec',fprec
!  write(6,*) 'gust_cv',gust_cv
!  write(6,*) 'gust',gust
!  write(6,*) 'b_star',b_star
!  write(6,*) 'q_star',q_star
!  write(6,*) '-------------------------'
...
!------------------------------------------------------------------------
   endif ! end if of do_moist_processes
!------------------------------------------------------------------------
...
end if ! end if of do_moist_processes_down
!*** yihsuan above should be moved to physics_down ***
...
!<--- flag11
!   set to large value
!     tdt(:,:,:)   = 0.1 !tdt(:,:,:)   + Tend_mf%ttnd  (:,:,:)  ! temperature       tendency
!     udt(:,:,:)   = 10. !udt(:,:,:)   + Tend_mf%utnd  (:,:,:)  ! u wind component  tendency
!     vdt(:,:,:)   = 10. !vdt(:,:,:)   + Tend_mf%vtnd  (:,:,:)  ! v wind component  tendency
!     rdt(:,:,:,1) = 0.01 !rdt(:,:,:,1) + Tend_mf%qtnd  (:,:,:)  ! specific humidity tendency 

!   set to large value
!     tdt(:,:,:)   = 0. !tdt(:,:,:)   + Tend_mf%ttnd  (:,:,:)  ! temperature       tendency
!     udt(:,:,:)   = 0. !udt(:,:,:)   + Tend_mf%utnd  (:,:,:)  ! u wind component  tendency
!     vdt(:,:,:)   = 0. !vdt(:,:,:)   + Tend_mf%vtnd  (:,:,:)  ! v wind component  tendency
!     rdt(:,:,:,1) = 0. !rdt(:,:,:,1) + Tend_mf%qtnd  (:,:,:)  ! specific humidity tendency 

! typical values
!     tdt(:,:,:)   = 1.e-5 !tdt(:,:,:)   + Tend_mf%ttnd  (:,:,:)  ! temperature       tendency
!     udt(:,:,:)   = 1.e-6 !udt(:,:,:)   + Tend_mf%utnd  (:,:,:)  ! u wind component  tendency
!     vdt(:,:,:)   = 1.e-6 !vdt(:,:,:)   + Tend_mf%vtnd  (:,:,:)  ! v wind component  tendency
!     rdt(:,:,:,1) = 1.e-8 !rdt(:,:,:,1) + Tend_mf%qtnd  (:,:,:)  ! specific humidity tendency 
...
!  diff_m(:,:,20:34) = 10.
  ...
  ! write(6,*) 'physics_down, diff_down_start, udt,',udt
  ! write(6,*) 'physics_down, diff_down_start, vdt,',vdt
  ! write(6,*) 'physics_down, diff_down_start, tdt,',tdt
  ! write(6,*) 'physics_down, diff_down_start, qdt,',rdt(:,:,:,1)
!<--- flag11
      ...
      if (do_clubb > 0) then
        ...
        if (do_EDMF_in_mass_flux) then
          call vert_diff_driver_down (is, js, Time_next, dt, p_half,   &
                                      p_full, z_full,   &
                                      diff_m_to_vdiff(is:ie,js:je,:),         &  ! yhc: diff_m_to_vdiff & diff_t_to_vdiff are zeros
                                      diff_t_to_vdiff(is:ie,js:je,:),         &  !      because ED is handled by mass_flux
                                      u ,v ,t ,r(:,:,:,1) ,r(:,:,:,1:ntp), &
                                      dtau_du, dtau_dv, tau_x, tau_y,  &
                                      udt, vdt, tdt, rdt(:,:,:,1), rdt,        &
                                      Surf_diff)
...
!--> yi-hsuan debug, 2020-07-02
!------- t tendency after vert diff down (units: K/s) at full level -------
      if ( id_tdt_after_vdiff_down > 0) then
        used = send_data (id_tdt_after_vdiff_down, tdt, Time_next, is, js, 1 )
      endif
!<-- yi-hsuan debug, 2020-07-02
...
!<--- flag222
  ...
  !  write(6,*) 'physics_down, end      , i,j,t',i,j,t(i,j,:),'end'
  !  write(6,*) 'physics_down, end      , i,j,tdt',i,i,tdt(i,j,:),'end'
  !end if
    !write(6,*) 'physics_down, end      , i,j,t',t(i,j,:)
    !write(6,*) 'physics_down, end      , i,j,tdt',tdt(i,j,:)
  !enddo
  !enddo

  ! write(6,*) 'physics_down, diff_down, u,',u
  ! write(6,*) 'physics_down, diff_down, v,',v
  ! write(6,*) 'physics_down, diff_down, t,',t
  ! write(6,*) 'physics_down, diff_down, q,',r(:,:,:,1)
  ! write(6,*) 'physics_down, diff_down, udt,',udt
  ! write(6,*) 'physics_down, diff_down, vdt,',vdt
  ! write(6,*) 'physics_down, diff_down, tdt,',tdt
  ! write(6,*) 'physics_down, diff_down, qdt,',rdt(:,:,:,1)
!<--- flag222

!<--- flag222
!   update MF tendencies here
!     tdt(:,:,:)   = tdt(:,:,:)   + Tend_mf%ttnd  (:,:,:)  ! temperature       tendency
!     udt(:,:,:)   = udt(:,:,:)   + Tend_mf%utnd  (:,:,:)  ! u wind component  tendency
!     vdt(:,:,:)   = vdt(:,:,:)   + Tend_mf%vtnd  (:,:,:)  ! v wind component  tendency
!     rdt(:,:,:,1) = rdt(:,:,:,1) + Tend_mf%qtnd  (:,:,:)  ! specific humidity tendency 
...
!  ! write(6,*) 'physics_down, mf_tend  , udt,',udt
!  ! write(6,*) 'physics_down, mf_tend  , vdt,',vdt
!  ! write(6,*) 'physics_down, mf_tend  , tdt,',tdt
!  ! write(6,*) 'physics_down, mf_tend  , qdt,',rdt(:,:,:,1)
!<--- flag222
...
type(surf_diff_type),   intent(inout)             :: Surf_diff
!real,dimension(:,:),    intent(out)               :: lprec, fprec   ! yihsuan
real,dimension(:,:),    intent(inout)               :: lprec, fprec  ! yihsuan set to inout because these may be computed in physics_down
...
3004 format (A35,2X,33(F10.3,2X,','),A5)
  ...
  ! write(6,*) 'physics_up  , beginning, q,',r(:,:,:,1)
  ! write(6,*) 'physics_up  , beginning, tdt,',tdt
  ! write(6,*) 'physics_up  , beginning, qdt,',rdt(:,:,:,1)
!<--- flag11
...
!<--- flag11
  ...
  !do j=js,je
  !if (i.eq.68 .and. j.eq.11) then
  !  write(6,*) 'physics_up  , end      , i,j,t',i,j,t(i,j,:),'end'
  !  write(6,*) 'physics_up  , end      , i,j,tdt',i,i,tdt(i,j,:),'end'
  !end if
  !enddo
  !enddo
!<--- flag11
...
!<--- yhc  if ED is done in mass_flux, do not call vert_diff_driver_up here
   if (.not.do_EDMF_in_mass_flux) then
      call vert_diff_driver_up (is, js, Time_next, dt, p_half,   &
                                Surf_diff, tdt, rdt(:,:,:,1), rdt )
   endif
!---> yhc

   !yhc comment   call vert_diff_driver_up (is, js, Time_next, dt, p_half,   &
   !yhc comment                             Surf_diff, tdt, rdt(:,:,:,1), rdt )

!--> yi-hsuan debug, 2020-07-02
!------- t tendency after vert diff up (units: K/s) at full level -------
      if ( id_tdt_after_vdiff_up > 0) then
        used = send_data (id_tdt_after_vdiff_up, tdt, Time_next, is, js, 1 )
      endif
!<-- yi-hsuan debug, 2020-07-02
...
!<--- flag11
  ...
  ! write(6,*) 'physics_up  , diff_up  , q,',r(:,:,:,1)
  ! write(6,*) 'physics_up  , diff_up  , tdt,',tdt
  ! write(6,*) 'physics_up  , diff_up  , qdt,',rdt(:,:,:,1)

! call error_mesg(' mod_name',' text', FATAL )   ! stop the model, This message would show in fms.out file
!<--- flag11
...
!---------------------------------------------------------------------
     ...
     if (do_mass_flux_after_vdiff) then
...
!<--- flag11
! write(6,*) 'is,js,ie,je',is,js,ie,je
!! write(6,*) 'b_star',b_star(1,1)
! write(6,*) 'Phys_mp_exch%pbltop',Phys_mp_exch%pbltop
! write(6,*) 'z_pbl',z_pbl

!       call mass_flux_driver ( is, js, dt, Time_next,                   &
!                               do_mass_flux_diagnostic, &
!                               p_half, p_full, &
!                               Physics_input_block%z_half, Physics_input_block%z_full, & 
!                               u_star, b_star, q_star, Phys_mp_exch%pbltop,           &
!                               Physics_input_block%u, Physics_input_block%v, &
!                               t, r(:,:,:,1), &
!                               Physics_tendency_block%u_dt, Physics_tendency_block%v_dt, &
!                               tdt, rdt(:,:,:,1) )
!<--- flag11
       ...
       call mpp_clock_end   ( mass_flux_clock )
     end if ! end if of do_mass_flux_after_vdiff

!*** yihsuan below should be moved to physics_down ***
if (do_moist_processes_up) then
...
!-----------------------------------------------------------------------
  ...
  if (do_writeout_column) then
    ...
    write(6,*) 'evap_star',evap_star(ii_write,jj_write)
    write(6,*) 'frac_land',frac_land(ii_write,jj_write)
        write(6,3001) 'data t_moist_up/'    ,t(ii_write,jj_write,:)
        write(6,3002) 'data q_moist_up/'    ,r(ii_write,jj_write,:,1)
        write(6,3002) 'data tdt_moist_up/'    ,tdt(ii_write,jj_write,:)
        write(6,3002) 'data qdt_moist_up/'    ,rdt(ii_write,jj_write,:,1)
       write(6,*) '-------------- i,j,',ii_write,jj_write
  endif
...
!  write(6,*) '-------------------------'
!  write(6,*) 'gg1, moist_processes, physics_up'
!  !write(6,*) 'Moist_clouds_block',Moist_clouds_block
!  !write(6,*) 'Physics_tendency_block',Physics_tendency_block
!  !write(6,*) 'Phys_mp_exch',Phys_mp_exch
!  write(6,*) 'lprec',lprec
...
end if ! end if of do_moist_processes_up
!*** yihsuan above should be moved to physics_down ***
...
!<--- flag22
  if (do_writeout_column) then
    ...
    write(6,*) 'u_star',u_star(ii_write,jj_write)
    write(6,*) 'q_star',q_star(ii_write,jj_write)
        write(6,3001) 'data t_physics_up_end/'    ,t(ii_write,jj_write,:)
        write(6,3002) 'data q_physics_up_end/'    ,r(ii_write,jj_write,:,1)
        write(6,3002) 'data tdt_physics_up_end/'    ,tdt(ii_write,jj_write,:)
        write(6,3002) 'data qdt_physics_up_end/'    ,rdt(ii_write,jj_write,:,1)
       write(6,*) '-------------- i,j,',ii_write,jj_write
  endif
