...
!--- option for mass_flux_tendencies solver
character*20 :: option_solver = "explicit"  
!character*20 :: option_solver = "implicit"

!--- compute phi_bar at half levels to calculate flux (now only available for the explicit solver)
!      centered-diff (default): centered-difference, i.e. phi_bar(k-1/2) = 0.5( phi_bar(k-1)+phi_bar(k) )
!      upwind                 : upwind-difference  , i.e. phi_bar(k-1/2) = phi_bar(k)
!character*20, parameter :: option_env_half = "upwind"         
character*20 :: option_env_half = "centered-diff"  
...
namelist / mass_flux_nml / up_num, do_mf_micro, option_solver, &
                           ...
                           option_env_half, &
                           use_tau_mf,   &
                           do_writeout_profile, do_printouts, do_stop_run
...
integer :: & 
           ...
           id_up_qt, id_up_qc, id_moist_massflux, id_is_massflux
...
!---------------------------------------------------------------------
  ...
  real, dimension(size(tt,1),size(tt,2)) :: &             ! dimension (nlon,nlat)
        ...
        evap_star,                          &             ! sensible heat flux retrieved from star values (W/m2)
...
!--------------------------------------
      ...
      shflx_star   (i,j) = w1_thv1_surf / (1.+d608*qq(i,j,kx)) * rho_full(i,j,kx) * cp_air
      evap_star    (i,j) = u_star(i,j) * q_star(i,j) * rho_full(i,j,kx)

! yihsuan
!  write(6,*) 'uuu1, mf_u_star',u_star
!  write(6,*) 'uuu1, mf_b_star',b_star
!  write(6,*) 'uuu1, mf_q_star',q_star

! diagnose shflx & lhflx for camparison with CM4
!write(6,*) 'vvv1, ex_flux_t, massflux',w1_thv1_surf*rho_full(i,j,kx)*cp_air / (qq(i,j,kx)*d608+1.0)
!write(6,*) 'vvv1, ex_flux_tr,massflux',w1_qt1_surf *rho_full(i,j,kx)*hlv
  ...
  if (do_writeout_profile) then
    ...
    do j=1,jx
      ...
      !if (tt1.eq.0.) then   
        ...
        write(6,3003) 'data w1_thv1_surf/',u_star(i,j) * b_star(i,j) * thv(i,j,kx)/g
        write(6,*)    ''
        write(6,*)    '! surface moisture flux (kg/kg m/s'
        write(6,3003) 'data w1_qt1_surf/',u_star(i,j) * q_star(i,j)
        write(6,*)    ''
        ...
        write(6,3001) 'data sum_up_massflux /'    ,sum_up_massflux(i,j,:)
        write(6,*)    ''
        ...
        write(6,3001) 'data w1_thli1 /'    ,w1_thli1(i,j,:)
        write(6,*)    ''
        write(6,*)    '! temperature tendency from MF (K/day) at full level'
